...
weekly_crime_aggregation = function(data, weeks, crime, aggr = T) {
    ...
    if (aggr == T) {
        # subset to cases in crime type
        result = data %>% # filter crimes in analysis weeks
        filter(ofns_desc %in% crime) %>% mutate(week = lubridate::isoweek(cmplnt_fr_dt)) %>% filter(week %in% weeks) %>% 
            group_by(week, loc_gid) %>% summarise(occ = n()) %>% rename(gid = loc_gid) %>% setorder(week, gid)
        setDT(result)
